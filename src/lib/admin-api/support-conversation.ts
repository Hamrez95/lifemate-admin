...
function parseMessages(value: unknown): { items: SupportConversationMessage[]; freshness: string } | null {
  if (!value || typeof value !== "object") return null;
  const body = value as Record<string, unknown>;
  if (!Array.isArray(body.items) || !body.freshness || typeof body.freshness !== "object") return null;
  ...
  for (const raw of body.items) {
    ...
    if (typeof row.senderAccountId !== "string" || !UUID_PATTERN.test(row.senderAccountId)) return null;
    if (!nullableString(row.senderDisplayName) || typeof row.body !== "string" || typeof row.createdAtUtc !== "string") return null;
    items.push(row as unknown as SupportConversationMessage);
  }
  return { items, freshness: freshness.asOfUtc };
}

export async function getSupportConversation(ticketId: string): Promise<ReadResult<{ items: SupportConversationMessage[]; freshness: string }>> {
...
export async function getSupportConversationOperations(ticketId: string): Promise<ReadResult<{ escalations: SupportConversationEscalation[]; links: SupportConversationLink[]; freshness: string }>> {
  if (!UUID_PATTERN.test(ticketId)) return { kind: "not_found" };
  const result = await requestJson(`/api/v1/support/tickets/${ticketId}/conversation/operations`);
  if (result.kind !== "ok") return result;
  const body = result.data as Record<string, unknown>;
  if (!Array.isArray(body.escalations) || !Array.isArray(body.links) || !body.freshness || typeof body.freshness !== "object") return { kind: "unavailable" };
  const freshness = body.freshness as Record<string, unknown>;
  ...
  for (const raw of body.escalations) {
    ...
    const row = raw as Record<string, unknown>;
    if (typeof row.escalationId !== "string" || !UUID_PATTERN.test(row.escalationId) || typeof row.status !== "string" || typeof row.safeReason !== "string" || typeof row.targetRoleCode !== "string" || typeof row.targetRoleName !== "string" || typeof row.createdAtUtc !== "string") return { kind: "unavailable" };
  ...
  for (const raw of body.links) {
    if (!raw || typeof raw !== "object") return { kind: "unavailable" };
    const row = raw as Record<string, unknown>;
    if (typeof row.linkId !== "string" || !UUID_PATTERN.test(row.linkId) || typeof row.linkKind !== "string" || !LINK_KINDS.has(row.linkKind) || typeof row.referenceCode !== "string" || typeof row.createdAtUtc !== "string") return { kind: "unavailable" };
    links.push(row as unknown as SupportConversationLink);
  }
  return { kind: "ok", data: { escalations, links, freshness: freshness.asOfUtc } };
}

async function mutate(ticketId: string, suffix: string, payload: Record<string, unknown>, idempotencyKey: string): Promise<ConversationMutationResult> {
  if (!UUID_PATTERN.test(ticketId)) return { kind: "not_found" };
  if (!IDEMPOTENCY_PATTERN.test(idempotencyKey)) return { kind: "invalid", message: "شناسه امن درخواست معتبر نیست." };
  const token = await accessToken();
  if (!token) return { kind: "unauthenticated" };
  const config = getPublicRuntimeConfig();
  let response: Response;
  try {
    response = await fetch(`${config.adminApiUrl}/api/v1/support/tickets/${ticketId}/conversation/${suffix}`, {
      method: "POST",
...
export function sendSupportConversationMessage(input: { ticketId: string; body: string; clientMessageId: string; idempotencyKey: string }) {
  return mutate(input.ticketId, "messages", { body: input.body, clientMessageId: input.clientMessageId }, input.idempotencyKey);
}

export function escalateSupportConversation(input: { ticketId: string; targetRoleCode: string; safeReason: string; idempotencyKey: string }) {
  return mutate(input.ticketId, "escalations", { targetRoleCode: input.targetRoleCode, safeReason: input.safeReason }, input.idempotencyKey);
}

export function linkSupportConversationReference(input: { ticketId: string; linkKind: SupportConversationLink["linkKind"]; referenceCode: string; idempotencyKey: string }) {
  return mutate(input.ticketId, "links", { linkKind: input.linkKind, referenceCode: input.referenceCode }, input.idempotencyKey);
}

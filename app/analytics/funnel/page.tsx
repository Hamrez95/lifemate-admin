...
function CanonicalMetric({ definition, value }: { definition: AnalyticsKpiDefinition; value: KpiValue }) {
  const normalized = value.value === null ? 0 : definition.unit === "rate" ? value.value * 100 : value.value;
  const width = definition.unit === "rate" ? Math.max(0, Math.min(normalized, 100)) : value.value === null ? 0 : 100;
...
async function FunnelContent({ filters }: { filters: URLSearchParams }) {
  ...
  if (valuesResult.kind === "invalid") {
    return <AdminPageState state="error" title="فیلتر قیف معتبر نیست" description="فیلتر تاریخ یا محصول را بررسی کنید." />;
  }
  if (catalogResult.kind !== "ok" || valuesResult.kind !== "ok") {
    const correlationId = catalogResult.kind === "unavailable" ? catalogResult.correlationId : valuesResult.kind === "unavailable" ? valuesResult.correlationId : undefined;
    return <AdminPageState state="unavailable" title="داده canonical قیف در دسترس نیست" description={correlationId ? `کد پیگیری: ${correlationId}` : undefined} />;
  }
  ...
  const measurable = catalog.kpis
    ...
    .filter((item): item is { definition: AnalyticsKpiDefinition; value: KpiValue } => Boolean(item.value));
    ...
    <div className={styles.page}>
      <section className={styles.hero}>
        <div>
          ...
          <p>
            این صفحه با الگوی مرجع ۹ طراحی شده، اما فقط داده‌ای را به‌عنوان «قیف» نشان می‌دهد که از قرارداد canonical قابل اثبات باشد. KPIهای مستقل به‌جای تبدیل مرحله‌به‌مرحله جعل نمی‌شوند.
          </p>
        </div>
      ...
      <section className={styles.toolbar} aria-label="کنترل‌های تحلیل قیف">
        <form method="get" className={styles.filters}>
          <label><span>از تاریخ</span><input type="date" name="from" defaultValue={values.query.from} /></label>
          <label><span>تا تاریخ</span><input type="date" name="to" defaultValue={values.query.to} /></label>
          <label><span>محصول</span><select name="product" defaultValue={values.query.product ?? ""}>{products.map((item) => <option key={item.value || "all"} value={item.value}>{item.label}</option>)}</select></label>
          <button type="submit">اعمال فیلتر</button>
        </form>
        <div className={styles.actions}>
          <button type="button" disabled title="endpoint canonical برای export قیف وجود ندارد">خروجی</button>
          <button type="button" disabled title="endpoint canonical برای drill-down قیف وجود ندارد">Drill-down</button>
        </div>
      </section>
      ...
      <section className={styles.metricsCard} aria-labelledby="canonical-evidence-title">
        ...
        <div className={styles.metricList}>
          {measurable.length === 0 ? (
            ...
            measurable.map(({ definition, value }) => <CanonicalMetric key={definition.name} definition={definition} value={value} />)
          )}
        </div>
      </section>
...
export default async function FunnelPage({ searchParams }: FunnelPageProps) {
  ...
  return (
    <AdminSessionProvider admin={admin}>
      <AdminShell activeSlug="analytics" title="قیف فعال‌سازی و تبدیل" subtitle="Reference 9 · canonical data only">

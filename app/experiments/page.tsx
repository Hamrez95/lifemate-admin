...
function banner(status: string): string | null {
  ...
  if (status === "unavailable")
    return "API فعلاً در دسترس نیست؛ fallback مستقیم به دیتابیس انجام نشد.";
  return null;
}
  ...
  return (
    <article className={styles.card}>
      ...
      <dl className={styles.metaGrid}>
        ...
        <div>
          <dt>Product</dt>
          <dd>{item.productCode ?? "Global"}</dd>
        </div>
        <div>
          <dt>Primary metric</dt>
          <dd>{item.primaryMetricCode}</dd>
        </div>
        <div>
          <dt>Version</dt>
      ...
      <td>
        {canWrite ? (
          <form action={feedbackAction} className={styles.feedbackForm}>
            ...
            <select
              name="feedbackAction"
              defaultValue={item.status === "Submitted" ? "Acknowledge" : "Triage"}
            >
            ...
            <input
              ...
              maxLength={500}
              required
              placeholder="دلیل / یادداشت عملیاتی"
            />
            <input name="supportTicketId" placeholder="Support ticket UUID (فقط LinkSupport)" />
            <input
              name="productIssueRef"
              maxLength={160}
              placeholder="Opaque issue ref (فقط LinkProductIssue)"
            />
...
export default async function ExperimentsFeedbackPage({ searchParams }: Props) {
    ...
    return (
      <AdminSessionProvider admin={admin}>
        <AdminShell
          ...
          title="Experiments & Feedback"
          subtitle="Product learning control plane"
        >
  ...
  return (
    <AdminSessionProvider admin={admin}>
      <AdminShell
        activeSlug="analytics"
        title="Experiments, Feedback & Advocacy"
        subtitle="Canonical product learning · no clinical experiments"
      >
        <div className={styles.content}>
          ...
          <section className={styles.hero}>
            <div>
              ...
              <p>
                Experiment فقط روی onboarding/pricing/paywall/CTA/offer/non-clinical UX اجرا می‌شود.
                Feedback از API canonical خوانده می‌شود و Advocacy فقط با opt-in صریح قابل بررسی
                است.
              </p>
            </div>
            ...
            <section className={styles.panel}>
              ...
              <form action={createExperimentAction} className={styles.formGrid}>
                <input
                  type="hidden"
                  name="idempotencyKey"
                  value={`experiment-create-${randomUUID()}`}
                />
                <label>
                  <span>Key</span>
                  <input name="experimentKey" pattern="[a-z][a-z0-9._-]{2,95}" required />
                </label>
                <label>
                  <span>Name</span>
                ...
                <label>
                  ...
                  <select name="surface">
                    ...
                    <option value="nonclinical_feature">nonclinical_feature</option>
                  </select>
                </label>
                <label>
                ...
                <label>
                  <span>Segment key (optional)</span>
                  <input name="segmentKey" />
                </label>
                <label>
                  <span>Segment snapshot UUID</span>
                  <input name="segmentSnapshotId" />
                </label>
                <label>
                  <span>Primary metric</span>
                  <input name="primaryMetricCode" required />
                </label>
                <label>
                  <span>Guardrails (comma separated)</span>
                  <input name="guardrailMetricCodes" />
                </label>
                <label>
                  <span>Start UTC ISO</span>
                  <input name="startsAtUtc" placeholder="2026-09-01T00:00:00.000Z" />
                </label>
                <label>
                  <span>End UTC ISO</span>
                  <input name="endsAtUtc" placeholder="2026-09-15T00:00:00.000Z" />
                </label>
                <label className={styles.wide}>
                  ...
                  <textarea
                    ...
                    required
                    defaultValue={
                      '[{"key":"control","weightBasisPoints":5000,"controlValue":false,"version":1},{"key":"variant","weightBasisPoints":5000,"controlValue":true,"version":1}]'
                    }
                  />
                </label>
                <label className={styles.wide}>

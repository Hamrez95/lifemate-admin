...
function resultMessage(result: { kind: string; message?: string; correlationId?: string }): string {
  if (result.kind === "forbidden") return "مجوز مدیریت Audience Segment برای این عملیات وجود ندارد.";
  if (result.kind === "conflict") return result.message ?? "Segment همزمان تغییر کرده است؛ صفحه را تازه کنید.";
  ...
  return result.correlationId ? `سرویس Audience در دسترس نیست. کد پیگیری: ${result.correlationId}` : "سرویس Audience فعلاً در دسترس نیست.";
}
...
function scalar(attribute: SegmentAttribute, operator: SegmentOperator, raw: string) {
  ...
  if (operator === "gte" || operator === "lte" || attribute === "engagement.last_active_days") {
    ...
    if (!Number.isFinite(value)) destination("error", "برای شرط عددی باید مقدار عددی معتبر وارد شود.");
    return value;
  }
  if (attribute === "product.enrolled") {
    if (raw !== "true" && raw !== "false") destination("error", "مقدار product.enrolled باید true یا false باشد.");
    return raw === "true";
  }
  if (operator === "in" || operator === "not_in") {
    const values = raw.split(",").map((item) => item.trim()).filter(Boolean);
    if (values.length < 1 || values.length > 50) destination("error", "فهرست شرط باید بین ۱ تا ۵۰ مقدار داشته باشد.");
    return values;
  }
...
export async function createAudienceSegmentAction(formData: FormData): Promise<void> {
  ...
  if (name.length < 2 || name.length > 120) destination("error", "نام Segment باید بین ۲ تا ۱۲۰ نویسه باشد.");
  if (description.length > 500) destination("error", "توضیح Segment بیش از حد طولانی است.");
  if (!attributes.includes(attribute)) destination("error", "Attribute انتخاب‌شده برای Marketing مجاز نیست.");
  ...
  destination("success", "Audience Segment ساخته شد؛ قبل از Campaign حتماً Preview/Snapshot را بررسی کنید.");
}
...
export async function snapshotAudienceSegmentAction(formData: FormData): Promise<void> {
  ...
  destination("success", `Snapshot immutable ساخته شد: ${result.data.id.slice(0, 8)}… · members=${count}`);
}

  ...
  return (
    <form action={action} className={styles.operationCard} aria-busy={pending}>
      ...
      <label>
        ...
        <input name="targetRoleCode" required minLength={2} maxLength={64} onChange={() => setNonce(crypto.randomUUID())} placeholder="engineering" />
      </label>
      <label>
        <span>دلیل عملیاتی امن</span>
        <textarea name="safeReason" required minLength={5} maxLength={800} rows={3} onChange={() => setNonce(crypto.randomUUID())} placeholder="بدون کپی اطلاعات سلامت یا تماس…" />
      </label>
      <Feedback state={state} />
...
function LinkForm({ ticketId, seed }: { ticketId: string; seed: string }) {
  ...
  return (
    <form action={action} className={styles.operationCard} aria-busy={pending}>
      ...
      <label>
        <span>نوع مرجع</span>
        <select name="linkKind" defaultValue="ProductIssue" onChange={() => setNonce(crypto.randomUUID())}>
      ...
      <label>
        ...
        <input name="referenceCode" required maxLength={180} onChange={() => setNonce(crypto.randomUUID())} placeholder="CORE-569 یا GH#569 — URL وارد نکنید" />
      </label>
      <Feedback state={state} />
  ...
  return (
    <section className={styles.workspace} aria-labelledby="support-conversation-title">
      ...
      <div className={styles.messageList} aria-live="polite">
        {ordered.length === 0 ? <p className={styles.empty}>هنوز پیامی در این گفتگو ثبت نشده است.</p> : ordered.map((message) => (
          <article className={styles.message} data-sender={message.senderKind} key={message.messageId}>
            <header>
              <strong>{message.senderDisplayName || (message.senderKind === "Staff" ? "تیم LifeMate" : "کاربر")}</strong>
      ...
      {canWrite ? <MessageComposer ticketId={ticketId} seed={`${requestSeed}-message`} /> : <p className={styles.readOnly}>برای پاسخ به کاربر مجوز support.write لازم است.</p>}

      <div className={styles.operationsGrid}>
        <div className={styles.operationHistory}>
          <strong>ارجاع‌ها</strong>
          {escalations.length === 0 ? <span>ارجاعی ثبت نشده</span> : escalations.map((item) => (
            <div key={item.escalationId}>
              <b>{item.targetRoleName}</b>
              <span>{item.status} · {formatDateTime(item.createdAtUtc)}</span>
        ...
        <div className={styles.operationHistory}>
          ...
          {links.length === 0 ? <span>مرجعی ثبت نشده</span> : links.map((item) => (
            <div key={item.linkId}>
              <b>{item.linkKind}</b>
      ...
      <aside className={styles.attachmentNotice}>
        ضمیمه‌ها فقط پس از وجود قرارداد signed-access در Core نمایش داده می‌شوند؛ این UI هرگز public bucket URL یا مسیر Storage مستقیم نمی‌سازد.
      </aside>
    </section>
  );
}

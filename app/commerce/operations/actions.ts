...
function common(formData: FormData):
  ...
  | { error: CommerceOperationsActionState } {
...
export async function requestRefundAction(
  ...
  const state = mapped(
    await requestCommerceRefund({ transactionId, amountMinor, ...shared }),
...
export async function openReconciliationAction(
  ...
  const caseType = text(formData, "caseType").toLowerCase();
  ...
  const state = mapped(
    await openCommerceReconciliationCase({ transactionId, caseType, ...shared }),

...
describe("workforce authentication transport boundary", () => {
  ...
  it("forwards only bounded invite-only auth actions to the public edge function", () => {
    const route = source("app/api/auth/workforce/route.ts");
    expect(route).toContain("MAX_BODY_BYTES = 16_384");
    expect(route).toContain('new Set(["login", "activate_founder"])');
    ...
    expect(route).toContain("Origin: browserOrigin");
    expect(route).toContain('request.headers.get("x-forwarded-host")');
    expect(route).toContain('request.headers.get("x-forwarded-proto")');

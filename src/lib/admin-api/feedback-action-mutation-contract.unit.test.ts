...
describe("feedback action mutation success contract", () => {
  ...
  it("keeps status unchanged for support/product linking actions", () => {
    ...
    expect(
      parseFeedbackActionSuccess(
        { ...base, action: "LinkSupport" },
        200,
        { itemId: ITEM_ID, expectedStatus: "Triaged", action: "LinkSupport" },
      )?.status,
    ).toBe("Triaged");
    expect(
      parseFeedbackActionSuccess(
        { ...base, action: "LinkProductIssue" },
        200,
        { itemId: ITEM_ID, expectedStatus: "Triaged", action: "LinkProductIssue" },
      )?.status,
    ).toBe("Triaged");
  });

...
import { initialAbuseActionState, retireAbuseRuleAction, upsertAbuseRuleAction } from "./actions";
...
function Message({ status, message }: { status: string; message?: string }) {
  ...
  return (
    <p className={styles.message} data-status={status} role="status">
      {message}
    </p>
  );
}
...
export function AbuseRuleForm({ canWrite }: { canWrite: boolean }) {
  ...
  return (
    <form action={action} className={styles.form}>
      ...
      <label>
        Code
        <input name="code" required placeholder="gift.daily_limit" />
      </label>
      <label>
        Context
        <input name="contextCode" required placeholder="gift" />
      </label>
      ...
      <label>
        Rule kind
        <select name="ruleKind" defaultValue="VelocityLimit">
          <option>VelocityLimit</option>
          <option>UsageCap</option>
          <option>Cooldown</option>
          <option>DuplicateKey</option>
          <option>EvidenceRequired</option>
        </select>
      </label>
      <label>
        Subject scope
        <select name="subjectScope" defaultValue="Account">
          <option>Account</option>
          <option>VerifiedPhone</option>
        </select>
      </label>
      <label>
        Action
        <select name="enforcementAction" defaultValue="RequireApproval">
          <option>Allow</option>
          <option>Deny</option>
      ...
      <label>
        Priority
        <input name="priority" type="number" min={1} max={10000} defaultValue={100} />
      </label>
      <label>
        Window seconds
        <input name="windowSeconds" type="number" min={1} max={31536000} />
      </label>
      <label>
        Max count
        <input name="maxCount" type="number" min={1} max={1000000} />
      </label>
      <label>
        Cooldown seconds
        <input name="cooldownSeconds" type="number" min={1} max={31536000} />
      </label>
      <label>
        Evidence code
        <input name="evidenceCode" placeholder="verified_evidence" />
      </label>
      <label>
        Approval request type
        <input name="approvalRequestType" placeholder="manual_review" />
      </label>
      <label>
        Expected version
        <input name="expectedVersion" type="number" min={1} placeholder="برای create خالی" />
      </label>
      <label className={styles.wide}>
        Reason
      ...
      <div className={styles.footer}>
        <button disabled={!canWrite || pending}>{pending ? "در حال ثبت…" : "ذخیره Rule"}</button>
        <span>Rule باید explainable باشد؛ black-box score در v1 وجود ندارد.</span>
      </div>

      ...
      >
        <div className={styles.page}>
          <section className={styles.hero} aria-labelledby="operations-title">
            ...
            <p>
              وضعیت سلامت، latency، job، deploy، provider و incident فقط از read model canonical Core
              نمایش داده می‌شود. هر بخشی که instrumentation معتبر ندارد صریحاً Unknown باقی می‌ماند.
            </p>
          </section>
              ...
              <section className={styles.grid3} aria-label="وضعیت قابلیت‌های عملیاتی">
                {result.snapshot.services.map((service) => (
                  <article key={service.key} className={styles.card}>
                    ...
                    <p><strong className={styles.codeSafe}>{service.key}</strong></p>
                    <p>{sourceLabel(service.source)}</p>
                    <p>
                      latency probe: {service.latencyMs === null ? "Unknown" : `${service.latencyMs} ms`}
                    </p>
                  </article>
                ))}
                ...
                <article className={styles.card}>
                  <header className={styles.cardHeader}>
                    ...
                    <span className={styles.badge}>{stateLabel(result.snapshot.backgroundJobs.state)}</span>
                  </header>
                ...
                <article className={styles.card}>
                  <header className={styles.cardHeader}>
                    ...
                    <span className={styles.badge}>{stateLabel(result.snapshot.deployments.state)}</span>
                  </header>
                ...
                <article className={styles.card}>
                  <header className={styles.cardHeader}>
                    ...
                    <span className={styles.badge}>{stateLabel(result.snapshot.providers.state)}</span>
                  </header>
                ...
                <article className={styles.card}>
                  <header className={styles.cardHeader}>
                    <h3>رخداد فعال</h3>
                    <span className={styles.badge}>{stateLabel(result.snapshot.incidents.state)}</span>
                  </header>
                  <p>{sourceLabel(result.snapshot.incidents.source)}</p>
                  <p>
                    active count: {result.snapshot.incidents.activeCount === null
                      ? "Unknown"
                ...
                <article className={styles.card}>
                  ...
                  <p>
                    UI مستقیماً provider یا health endpoint عمومی را probe نمی‌کند و هیچ uptime، error-rate
                    یا success-rate را از داده‌های ناقص استنتاج نمی‌کند.
                  </p>
                </article>
              </section>
            </>
          ...
          <section className={styles.panel} aria-labelledby="operations-paths-title">
            ...
            <p>
              این لینک‌ها فقط به workspaceهای موجود می‌روند؛ هیچ endpoint عملیاتی bypass یا direct browser
              probe ایجاد نشده است.
            </p>

...
import { startPerformanceAdminApiProxy } from "./performance-admin-api-proxy.mjs";
...
const { server: performanceProxyServer, origin: performanceProxyOrigin } =
  await startPerformanceAdminApiProxy(canonicalOrigin);
...
const child = spawn(process.execPath, [path.join(standaloneRoot, "server.js")], {
  ...
  env: {
    ...
    NEXT_PUBLIC_ADMIN_API_URL: performanceProxyOrigin,
...
async function shutdown(signal) {
  ...
  await Promise.all([
    ...
    closeServer(workforceAuthServer),
    closeServer(performanceProxyServer),
  ]);
}

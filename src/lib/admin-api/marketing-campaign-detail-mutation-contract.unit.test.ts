...
describe("marketing campaign detail mutation success contract", () => {
  ...
  it("binds approval to the requested Approved/Revoked target", () => {
    ...
    expect(
      isMarketingCampaignDetailMutationSuccess(
        { ...base, approvalState: "Approved" },
        200,
        { kind: "approval", campaignId: CAMPAIGN_ID, approved: true },
      ),
    ).toBe(true);
    expect(
      isMarketingCampaignDetailMutationSuccess(
        { ...base, approvalState: "Revoked" },
        200,
        { kind: "approval", campaignId: CAMPAIGN_ID, approved: false },
      ),
    ).toBe(true);
    expect(
      isMarketingCampaignDetailMutationSuccess(
        { ...base, approvalState: "Pending" },
        200,
        { kind: "approval", campaignId: CAMPAIGN_ID, approved: true },
      ),
    ).toBe(false);

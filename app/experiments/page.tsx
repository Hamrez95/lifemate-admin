      ...
      <div className={styles.variantList}>
        {item.variants.map((variant) => (
          <span key={variant.key}>
            {variant.key}: {(variant.weightBasisPoints / 100).toLocaleString("fa-IR")}٪
          </span>
        ))}
      </div>
      {canWrite ? (
        ...
        {canWrite ? (
          <form action={feedbackAction} className={styles.feedbackForm}>
            ...
            <select name="feedbackAction" defaultValue={item.status === "Submitted" ? "Acknowledge" : "Triage"}>
              ...
              <option value="LinkSupport">Link Support</option>
              <option value="LinkProductIssue">Link Product Issue</option>
            </select>
  ...
  if (!canReadExperiments && !canReadFeedback) {
    return (
      <AdminSessionProvider admin={admin}>
        <AdminShell activeSlug="analytics" title="Experiments & Feedback" subtitle="Product learning control plane">
          <AdminPageState state="forbidden" title="مجوز Product Signals در دسترس نیست" />
        </AdminShell>
        ...
        <div className={styles.content}>
          ...
          <section className={styles.hero}>
            <div>
              ...
              <h2>یادگیری محصول بدون ساخت داده یا دور زدن Consent</h2>
              <p>Experiment فقط روی onboarding/pricing/paywall/CTA/offer/non-clinical UX اجرا می‌شود. Feedback از API canonical خوانده می‌شود و Advocacy فقط با opt-in صریح قابل بررسی است.</p>
            </div>
            <div className={styles.rules}>
              <span>Feature Flags are authoritative</span>
              <span>No health-data experiment targeting</span>
              <span>No social scraping</span>
            </div>
          </section>
          ...
          {canWriteExperiments ? (
            <section className={styles.panel}>
              ...
              <form action={createExperimentAction} className={styles.formGrid}>
                ...
                <label><span>Guardrails (comma separated)</span><input name="guardrailMetricCodes" /></label>
                <label><span>Start UTC ISO</span><input name="startsAtUtc" placeholder="2026-09-01T00:00:00.000Z" /></label>
                <label><span>End UTC ISO</span><input name="endsAtUtc" placeholder="2026-09-15T00:00:00.000Z" /></label>
          ...
          <section className={styles.panel}>
            <div className={styles.heading}><h3>Experiments</h3><span>{experiments?.kind === "ok" ? experiments.data.total.toLocaleString("fa-IR") : "—"}</span></div>
            {experiments?.kind === "ok" ? (
              experiments.data.items.length ? <div className={styles.cards}>{experiments.data.items.map((item) => <ExperimentCard key={item.key} item={item} canWrite={canWriteExperiments} />)}</div> : <AdminPageState state="empty" title="Experimentی وجود ندارد" />
            ) : <AdminPageState state="unavailable" title="Experiment API در دسترس نیست" description="هیچ Feature Flag یا نتیجه‌ای به‌صورت local جعل نمی‌شود." />}
          </section>

          <section className={styles.panel}>
            <div className={styles.heading}><h3>Feedback / NPS / Advocacy</h3><span>{feedback?.kind === "ok" ? feedback.data.total.toLocaleString("fa-IR") : "—"}</span></div>
            {feedback?.kind === "ok" ? (
              <div className={styles.tableWrap}><table><thead><tr><th>Kind</th><th>Product</th><th>Version</th><th>NPS</th><th>Status</th><th>Message</th><th>Action</th></tr></thead><tbody>{feedback.data.items.map((item) => <FeedbackRow key={item.itemId} item={item} canWrite={canWriteFeedback} />)}</tbody></table></div>
            ) : <AdminPageState state="unavailable" title="Feedback Admin API هنوز آماده نیست" description="این بخش تا merge شدن Core #574 fail-closed می‌ماند؛ direct DB fallback وجود ندارد." />}
          </section>

          <section className={styles.panel}>
            <div className={styles.heading}><h3>۳۰ روز اخیر</h3><span>Aggregate only</span></div>
            {trends?.kind === "ok" ? (
              <div className={styles.trends}>{trends.data.items.slice(0, 24).map((item, index) => <div key={`${item.day}-${item.product_code}-${item.kind}-${item.status}-${index}`}><strong>{String(item.item_count)}</strong><span>{item.day} · {item.product_code} · {item.kind} · {item.status}</span><small>NPS responses: {String(item.nps_response_count)} · avg: {item.average_nps ?? "—"}</small></div>)}</div>
            ) : <p className={styles.muted}>Trend فقط از aggregate canonical نمایش داده می‌شود.</p>}
            <p className={styles.muted}>Advocacy reward execution هنوز تا وجود API canonical reward handoff غیرفعال است؛ review یا view جعلی ساخته نمی‌شود.</p>
          </section>
        </div>
      </AdminShell>
    </AdminSessionProvider>

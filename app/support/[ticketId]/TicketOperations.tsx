...
function NoteForm({ ticketId, seed }: { ticketId: string; seed: string }) {
  const [state, action, pending] = useActionState(runSupportTicketAction, initialSupportActionFormState);
  ...
  return (
    <form action={action} className={styles.operationCard} aria-busy={pending}>
      ...
      <div className={styles.operationHeading}>
        <span className={styles.operationIcon} data-tone="violet" aria-hidden="true">✎</span>
        <div><strong>یادداشت داخلی</strong><small>فقط برای تیم پشتیبانی</small></div>
      </div>
      <label className={styles.field}>
        <span>متن یادداشت</span>
        <textarea name="note" required minLength={10} maxLength={2000} rows={4} disabled={pending} placeholder="خلاصه عملیاتی بدون اطلاعات سلامت، تماس یا جزئیات حساس غیرضروری…" onChange={request.rotate} />
        <small>متن یادداشت در Audit metadata کپی نمی‌شود؛ فقط اطلاعات حداقلی و غیرپزشکی ثبت کنید.</small>
      </label>
...
function StatusForm({ ticketId, current, seed }: { ticketId: string; current: string; seed: string }) {
  ...
  return (
    <form action={action} className={styles.operationCard} aria-busy={pending}>
      ...
      <input type="hidden" name="idempotencyKey" value={request.key} />
      <div className={styles.operationHeading}>
        <span className={styles.operationIcon} data-tone="green" aria-hidden="true">✓</span>
        <div><strong>وضعیت تیکت</strong><small>چرخه رسیدگی audited</small></div>
      </div>
      <label className={styles.field}>
...
function PriorityForm({ ticketId, current, seed }: { ticketId: string; current: string; seed: string }) {
  const [state, action, pending] = useActionState(runSupportTicketAction, initialSupportActionFormState);
  const request = useIdempotencyKey(seed);
  return (
    <form action={action} className={styles.operationCard} aria-busy={pending}>
      ...
      <input type="hidden" name="idempotencyKey" value={request.key} />
      <div className={styles.operationHeading}>
        <span className={styles.operationIcon} data-tone="orange" aria-hidden="true">!</span>
        <div><strong>اولویت</strong><small>ترتیب رسیدگی</small></div>
      </div>
      <label className={styles.field}>
        <span>اولویت جدید</span>
        <select name="priority" defaultValue={current} disabled={pending} onChange={request.rotate}>
          <option value="Low">پایین</option><option value="Normal">عادی</option><option value="High">بالا</option><option value="Urgent">فوری</option>
        </select>
      </label>
      <Feedback state={state} />
...
function AssigneeForm({ ticketId, current, assignees, seed }: { ticketId: string; current: string | null; assignees: SupportAssignee[]; seed: string }) {
  const [state, action, pending] = useActionState(runSupportTicketAction, initialSupportActionFormState);
  const request = useIdempotencyKey(seed);
  const options = useMemo(() => assignees.map((assignee) => ({ value: assignee.accountId, label: assignee.displayName || `Admin ${assignee.accountId.slice(0, 8)}…` })), [assignees]);
  return (
    <form action={action} className={styles.operationCard} aria-busy={pending}>
      <input type="hidden" name="ticketId" value={ticketId} />
      ...
      <input type="hidden" name="idempotencyKey" value={request.key} />
      <div className={styles.operationHeading}>
        <span className={styles.operationIcon} data-tone="blue" aria-hidden="true">↗</span>
        <div><strong>مسئول رسیدگی</strong><small>عضو فعال تیم پشتیبانی</small></div>
      </div>
      <label className={styles.field}>
        <span>تخصیص به</span>
        <select name="assigneeAccountId" defaultValue={current ?? ""} disabled={pending} onChange={request.rotate}>
...
export function TicketOperations({ ticketId, status, priority, assignedAdminAccountId, canWrite, assignees, requestSeed }: TicketOperationsProps) {
  if (!canWrite) {
    return (
      <section className={styles.readOnlyPanel} aria-label="عملیات تیکت">
        <div><span>Support operations</span><strong>حالت فقط مشاهده</strong><p>برای یادداشت، تغییر وضعیت، اولویت یا مسئول تیکت، مجوز support.write لازم است.</p></div>
  ...
  return (
    <section className={styles.operations} aria-labelledby="ticket-operations-title">
      <header className={styles.sectionHeading}>
        <div><span>Audited actions</span><h3 id="ticket-operations-title">عملیات تیکت</h3><p>فقط عملیات پشتیبانی‌شده Admin API؛ idempotent، permission-checked و audit-ready.</p></div>
      </header>
      <div className={styles.operationGrid}>
        ...
        <AssigneeForm ticketId={ticketId} current={assignedAdminAccountId} assignees={assignees} seed={`${requestSeed}:assignee`} />
      </div>
    </section>
  );
}

...
export type StaffActionFormState = {
...
export async function runStaffAction(
  _previous: StaffActionFormState,
  formData: FormData,
): Promise<StaffActionFormState> {
  ...
  const result = await performStaffAction({
    accountId,
    action,
  ...
  if (result.kind === "ok") {
    ...
    return {
      ...
      message: result.data.noop
        ? "وضعیت از قبل مطابق درخواست بود؛ نتیجه امن درخواست بازیابی شد."
        : "تغییر با موفقیت ثبت شد و رویداد audit ایجاد شد.",
    };
  }
  ...
  if (result.kind === "forbidden")
    return {
      status: "forbidden",
      message: result.message ?? "این تغییر مجاز نیست؛ تغییر خود، Founder و Super Admin مسدود است.",
    };
  if (result.kind === "conflict")
    return {
      status: "conflict",
      message: result.message ?? "وضعیت فعلی با درخواست سازگار نیست؛ صفحه را تازه‌سازی کنید.",
    };
  if (result.kind === "not_found")
    return { status: "conflict", message: result.message ?? "عضو یا نقش پیدا نشد." };
  if (result.kind === "invalid")
    return { status: "invalid", message: result.message ?? "اطلاعات عملیات معتبر نیست." };
  return {
    status: "unavailable",
    message: result.correlationId
      ? `سرویس در دسترس نیست. کد پیگیری: ${result.correlationId}`
      : "سرویس عملیات کارکنان در دسترس نیست؛ دوباره تلاش کنید.",
  };
}

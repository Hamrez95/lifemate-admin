...
import { ReconciliationForm, RefundRequestForm, RenewalIntentForm } from "./PaymentOperationsControls";
...
export default async function CommerceOperationsPage() {
  ...
  if (!relevant.some((permission) => admin.permissions.includes(permission))) redirect("/forbidden");
        ...
        <main className={styles.page} dir="rtl">
          ...
          <section className={styles.grid}>
            <article className={styles.panel}>
              <header><div><p className="eyebrow">Refunds</p><h3>Refund Requests</h3></div><span>{snapshot.refunds?.length ?? 0}</span></header>
              {snapshot.refunds?.slice(0, 8).map((item) => (
                <div className={styles.row} key={item.refundRequestId}>
                  <div><strong>{item.amountMinor} {item.currency}</strong><small>{item.requestStatus} · {item.providerStatus ?? "provider pending"}</small></div>
                  <time>{format(item.requestedAtUtc)}</time>
                </div>
              )) ?? <p className={styles.muted}>داده refund برای این session قابل مشاهده نیست.</p>}
              {canRefund ? <RefundRequestForm /> : <p className={styles.muted}>permission: commerce.refund.request لازم است.</p>}
            </article>

            <article className={styles.panel}>
              <header><div><p className="eyebrow">Reconciliation</p><h3>Provider mismatch cases</h3></div><span>{snapshot.reconciliationCases?.length ?? 0}</span></header>
              {snapshot.reconciliationCases?.slice(0, 8).map((item) => (
                <div className={styles.row} key={item.caseId}>
                  <div><strong>{item.caseType}</strong><small>{item.status} · {item.classificationSource ?? item.source}</small></div>
                  <time>{format(item.openedAtUtc)}</time>
                </div>
              )) ?? <p className={styles.muted}>Reconciliation facts برای این session قابل مشاهده نیست.</p>}
              {canReconcile ? <ReconciliationForm /> : <p className={styles.muted}>permission: commerce.reconciliation.write لازم است.</p>}
            </article>

            <article className={styles.panel}>
              <header><div><p className="eyebrow">Churn</p><h3>Non-renewal intents</h3></div><span>{snapshot.churn?.length ?? 0}</span></header>
              {snapshot.churn?.slice(0, 8).map((item) => (
                <div className={styles.row} key={item.subscriptionId}>
                  <div><strong>{item.cancellationReasonCode ?? "reason unavailable"}</strong><small>{item.status} · {item.cancelAtPeriodEnd ? "cancel at period end" : "renewal active"}</small></div>
              ...
              {canChurn ? <RenewalIntentForm /> : <p className={styles.muted}>permission: commerce.churn.write لازم است.</p>}
            </article>
          </section>

          <footer className={styles.footer}>As of {format(snapshot.asOfUtc)} · هیچ revenue/refund/provider fact از UI استنتاج نمی‌شود.</footer>
        </main>
      </AdminShell>
